...
async def _(event):
    ...
    OUTPUT = f"**⌔∮ [⌯╼═══❬ ❁ ᥲ️᥎ᥲ️ƚᥲ️ᖇ ᥉᥆ᥙᖇᥴᥱ ❁ ❭═══╾⌯](tg://FTTUTY/) الالاضافات:**\n{o}"
...
async def _(event):
    ...
    o = stdout.decode()
    OUTPUT = (
        f"**[⌯╼═══❬ ❁ ᥲ️᥎ᥲ️ƚᥲ️ᖇ ᥉᥆ᥙᖇᥴᥱ ❁ ❭═══╾⌯](tg://FTTUTY/) - فـارات تنصيبـك هـي:**\n\n\n{o}"
    )
    if len(OUTPUT) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            out_file.name = "env.text"
...
async def _(event):
    ...
    stdout, stderr = await process.communicate()
    o = stdout.decode()
    OUTPUT = f"**[ᯓ ⌯╼═══❬ ❁ ᥲ️᥎ᥲ️ƚᥲ️ᖇ ᥉᥆ᥙᖇᥴᥱ ❁ ❭═══╾⌯](tg://source_av/) - سرعـة السيرفـر**\n**- تم حسـاب سرعـة سيرفـر البـوت الخـاص بك :**\n\n{o}"
    if len(OUTPUT) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            out_file.name = "env.text"
            await event.client.send_file(
...
async def zeddd(event):
    ...
    zedt = "**- تاريخ تنصيبـك لـ بـوت أفــاتــار - 𓆩⌯╼═══❬ ❁ ᥲ️᥎ᥲ️ƚᥲ️ᖇ ᥉᥆ᥙᖇᥴᥱ ❁ ❭═══╾⌯𓆪**\n\n"
