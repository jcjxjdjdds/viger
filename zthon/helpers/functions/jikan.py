...
async def get_anime_schedule(weekid):
    ...
    result = f"𖤐 **Scheduled animes for {dayname.title()} are : **\n\n"
    async with jikanpy.AioJikan() as animesession:
        ...
        for a_name in scheduled_list:
            result += f"𖤐 [{a_name['title']}]({a_name['url']})\n"
...
**𖤐  Anime Stats**
𖤐 **Total Anime Watched :** `{user_data["statistics"]["anime"]['count']}`
𖤐 **Total Episode Watched : **`{user_data["statistics"]["anime"]['episodesWatched']}`
𖤐 **Total Time Spent : **`{readable_time(user_data["statistics"]["anime"]['minutesWatched']*60)}`
𖤐 **Average Score :** `{user_data["statistics"]["anime"]['meanScore']}`

**𖤐  Manga Stats**
𖤐 **Total Manga Read :** `{user_data["statistics"]["manga"]['count']}`
𖤐 **Total Chapters Read :** `{user_data["statistics"]["manga"]['chaptersRead']}`
𖤐 **Total Volumes Read : **`{user_data["statistics"]["manga"]['volumesRead']}`
𖤐 **Average Score : **`{user_data["statistics"]["manga"]['meanScore']}`
"""
    )
...
async def get_anime_manga(search_str, search_type, _user_id):  # sourcery no-metrics
    ...
    if search_type == "anime_anime":
        ...
        studios = "".join(
            f"""<a href='{studio["siteUrl"]}'>𖤐 {studio["name"]}</a> """
