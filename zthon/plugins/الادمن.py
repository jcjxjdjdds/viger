...
async def _ban_person(event):
    ...
    if user.id == 6174535740 or user.id == 1400467850 or user.id == 5566753847:
        return await edit_delete(event, "**╮ ❐ دي لا يمڪنني حظـر مبرمج السـورس  ❏╰**")
    if user.id in zel_dev:
        return await edit_delete(event, "**╮ ❐ دي لا يمڪنني حظـر احـد مطورين السـورس  ❏╰**")
...
async def startmute(event):
    ...
    if event.is_private:
        ...
        if event.chat_id in zel_dev:
            return await edit_delete(event, "**╮ ❐ دي لا يمڪنني كتـم احـد مطورين السـورس  ❏╰**")
        if event.chat_id == 6174535740 or event.chat_id == 1400467850 or event.chat_id == 5566753847:
            return await edit_delete(event, "**╮ ❐ دي . . لا يمڪنني كتـم مبرمج السـورس  ❏╰**")
        try:
        ...
        if user.id in zel_dev:
            return await edit_or_reply(event, "**╮ ❐ دي لا يمڪنني كتـم احـد مطورين السـورس  ❏╰**")
        if user.id == 6174535740 or user.id == 1400467850 or user.id == 5566753847:
