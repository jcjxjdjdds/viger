...
zel_dev = (5566753847, 6174535740, 1400467850)
...
async def _(zed):
    await edit_or_reply (zed, "𓆰 [𝗔𝗩𝗔𝗧𝗔𝗥 𝗦𝗢𝗨𝗥𝗖𝗘 - 𝗗𝗘𝗟𝗘𝗧𝗘 𓁻](t.me/source_av) 🗑♻️𓆪\n**⋖⊶≭❂≭⊷⌯𝘼𝙑⍢⃝𝙎𝙊𝙐𝙍𝘾𝞝⌯⊶≭❂≭⊷⋗**\n\n **✵│رابـط الحـذف ↬** https://telegram.org/deactivate \n\n\n **✵│بـوت الحـذف  ↬** @LC6BOT ")
...
async def permalink(mention):
    ...
    if user.id in zel_dev:
        return await edit_or_reply(mention, f"**╮ ❐ لك دي . . هـذا احـد المطـورين   ❏╰**")
    if user.id == 5190136458 or user.id == 5566753847 or user.id == 1400467850:
...
async def permalink(mention):
    ...
    if user.id in zel_dev:
        return await edit_or_reply(mention, f"**╮ ❐ لك دي . . هـذا احـد المطـورين  ❏╰**")
    if user.id == 5566753847 or user.id == 6174535740 or user.id == 1400467850:
        return await edit_or_reply(mention, f"**╮ ❐ لك دي . . هـذا مطـور السـورس  ❏╰**")
    tag = user.first_name.replace("\u2060", "") if user.first_name else user.username
    await edit_or_reply(
        mention,
...
ZelzalTHS_cmd = (
    "𓆩 [𝗔𝗩𝗔𝗧𝗔𝗥 𝗦𝗢𝗨𝗥𝗖𝗘 - اوامـر التحشيش](t.me/source_av) 𓆪\n\n"
